...
for i=1:10
        [accuracy_train_LR(1,i),accuracy_test_LR(1,i),tmp,predicted_class_LR(:,i)]=LogisticRegression(XTrain(:,:,i),XTest(:,:,i),yTrain(:,:,i),yTest(:,:,i));
end
for i=1:length(feature_list)
         acc_test_LR=mean(accuracy_test_LR(1,:));
         acc_train_LR=mean(accuracy_train_LR(1,:));
end
...
function [accuracy_train_LR,accuracy_test_LR,tmp,predicted_class]=LogisticRegression(XTrain,XTest,yTrain,yTest)
